...
from modules.spade_utils import pytorch_xavier_weight_factor, pytorch_kaiming_weight_factor
...
def spade(segmap, x_init, channels, use_bias=True, sn=False, scope='spade') :
    with tf.variable_scope(scope) :
        ...
        segmap_down = down_sample(segmap, x_h, x_w)
...
def up_sample(x, new_h, new_w):
    # _, h, w, _ = x.get_shape().as_list()
    # new_size = [h * scale_factor, w * scale_factor]
    return tf.image.resize_bilinear(x, size=(new_h, new_w))

def down_sample(x, new_h, new_w) :
    # _, h, w, _ = x.get_shape().as_list()
    # new_size = [h // scale_factor_h, w // scale_factor_w]

    return tf.image.resize_nearest_neighbor(x, size=(new_h, new_w))
